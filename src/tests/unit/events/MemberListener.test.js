...
import { MemberListener } from '../../../core/events/MemberListener.js';

describe('MemberListener', () => {
    let listener;
    let mockContainer;
    let mockRegistry;
    let mockLogger;
    ...
    beforeEach(() => {
        ...
        listener = new MemberListener(mockContainer, mockRegistry, mockLogger);
    });
    ...
    describe('dispatchEvent', () => {
        it('应该跳过无处理器的事件', async () => {
            ...
            await listener.dispatchEvent('guildMemberAdd', {});
        ...
        it('应该执行所有处理器', async () => {
            ...
            await listener.dispatchEvent('guildMemberAdd', eventData);
        ...
        it('应该跳过filter返回false的处理器', async () => {
            const handler1 = {
                id: 'handler1',
                filter: vi.fn(() => false),
                handle: vi.fn().mockResolvedValue({})
        ...
        it('应该处理guildMemberUpdate事件', async () => {
            const handler = {
                id: 'handler1',
                handle: vi.fn().mockResolvedValue({})
            };
            ...
            const oldMember = { guild: { id: 'guild123' } };
            const newMember = { guild: { id: 'guild123' } };

            await listener.dispatchEvent('guildMemberUpdate', {
                oldMember,
                newMember
            });
        ...
        it('应该记录debug日志', async () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith({
                msg: '处理事件: guildMemberAdd',
                handlersCount: 1
            });

            expect(mockLogger.debug).toHaveBeenCalledWith({
                msg: '事件处理完成: guildMemberAdd',
                handlerId: 'handler1'
            });
        });
    });
