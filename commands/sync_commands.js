    ...
    async execute(interaction, guildConfig) {
        ...
        try {
            ...
            if (commandsToDelete.length > 0) {
                statusReport.push(`需要删除 ${commandsToDelete.length} 个命令: ${commandsToDelete.map(cmd => cmd.name).join(', ')}`);
            }
            if (commandsToUpdate.length > 0) {
                statusReport.push(`需要更新 ${commandsToUpdate.length} 个命令: ${commandsToUpdate.map(cmd => cmd.name).join(', ')}`);
            }
