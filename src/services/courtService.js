...
class CourtService {
    ...
    static async createDebateThread(process, guildConfig, client) {
        ...
        const debateThread = await debateForum.threads.create({
            name: `对 ${target?.username || '未知用户'} 的${process.details.embed?.title?.replace('申请', '辩诉') || '辩诉帖'}`,
            message: {
                embeds: [{
                    ...(process.details.embed || {}),
                    title: `对 ${target?.tag || '未知用户'} 的${process.details.embed?.title?.replace('申请', '辩诉') || '辩诉帖'}`,
                    fields: [
                        ...(process.details.embed?.fields?.filter(f => f) || []),
                    ],
                }],
            },
    ...
    static async handleProcessExpiry(process, client) {
        try {
            ...
            if (!process.type.startsWith('court_') &&
                !process.type.startsWith('appeal') &&
                process.type !== 'vote') {
                return;
            }
            ...
            // 获取主服务器配置
            const mainGuildConfig = client.guildManager.getGuildIds()
            ...
            if (currentProcess.type === 'vote' && details.threadId) {
                await client.channels.fetch(details.threadId)
                    .then(thread => thread?.messages.fetch(currentProcess.statusMessageId))
                    .then(statusMessage => statusMessage?.edit({
                        embeds: [{
                            color: 0xFF0000,
                            title: '📢 议事投票已过期',
                            description: [
	        ...
	        switch (process.type) {
	            ...
	            case 'appeal': {
	                ...
	                if (mainGuild && guildConfig.courtSystem.appealDebateRoleId) {
	                    ...
	                    if (targetMember) {
	                        await targetMember.roles.add(guildConfig.courtSystem.appealDebateRoleId, '上诉申请通过')
