...
export default {
    cooldown: 30,
    data: new SlashCommandBuilder().setName('同步永封').setDescription('同步所有服务器的永封列表'),
    ...
    async execute(interaction, guildConfig) {
        ...
        try {
            ...
            await handleConfirmationButton({
                interaction,
                customId: 'confirm_sync_bans',
                buttonLabel: '确认同步',
                embed: {
                    color: 0xff0000,
                    title: '⚠️ 操作确认',
                    description:
                        '你确定要同步所有服务器的永封列表吗？\n\n**⚠️ 警告：此操作将在所有服务器之间同步永封用户！**',
                    fields: [
                        {
                            name: '操作',
                            value: '同步永封列表',
                ...
                onConfirm: async confirmation => {
                    ...
                    // 更新进度
                    await interaction.editReply(
                        ['⏳ 正在分析永封差异...', '**永封列表获取结果：**', guildBanFetchResults.join('\n')].join(
                            '\n',
                        ),
                    );
                        ...
                        try {
                            ...
                            for (const guildId of task.notBannedIn) {
                                try {
                                    ...
                                    logTime(
                                        `在服务器 ${guildId} 同步永封用户 ${user.tag} 失败: ${error.message}`,
                                        true,
                                    );
                                }
                            }
                        ...
                        embeds: [
                            {
                                color: 0x808080,
                                title: '❌ 确认已超时',
                                description: '同步永封操作已超时。如需继续请重新执行命令。',
                            },
                        ],
                        components: [],
                    });
